...
namespace Titled_Gui.Modules.Rage
{
    public class Aimbot : Classes.ThreadService
    {
        ...
        // Śledzenie strzałów - tylko do debugowania
        private static int lastAimbotShotsFired = 0;
        private static float lastAimbotShotTime = 0f;
        private const float AIMBOT_SHOT_COOLDOWN = 0.15f;

        public static void EnableAimbot()
        {
            try
            {
                if (!AimbotEnable || ((User32.GetAsyncKeyState(AimbotKey) & 0x8000) == 0) || Entities == null || GameState.LocalPlayer.Health == 0 || (ScopedOnly && !GameState.LocalPlayer.IsScoped) || (FlashCheck && GameState.LocalPlayer.IsFlashed))
                {
                    RandomChosen = false;
                    return;
                }
                ...
                if (useHeadPosition) //bone pos first
                {
                    ...
                    else
                    {
                        if (target != null)
                            ...
                            newAngles = new Vector2(GameState.swed.ReadVec(client, Offsets.dwViewAngles).X, GameState.swed.ReadVec(client, Offsets.dwViewAngles).Y);
                    }
                }
                ...
                // Sprawdź czy gracz strzela podczas aimowania
                CheckIfShootingWhileAiming();
                    ...
                    case 0: // mouse
                        {
                            ...
                            ? target.Bones2D[CurrentBoneIndex] : target?.Position2D ?? Vector2.Zero;
                    ...
                    case 1: // write
                        {
                            ...
                            GameState.swed.WriteVec(client, Offsets.dwViewAngles, newAnglesVec3);
                        }
                        break;
                }
            }
        ...
        private static void CheckIfShootingWhileAiming()
        {
            try
            {
                ...
                // Pobierz aktualną liczbę strzałów bezpośrednio z pamięci
                ...
                // Sprawdź czy liczba strzałów się zwiększyła
                if (currentShotsFired > lastAimbotShotsFired &&
                    (currentTime - lastAimbotShotTime) > AIMBOT_SHOT_COOLDOWN)
                {
                    // Powiadom BulletTrailManager o strzale
                    BulletTrailManager.OnShootDetected();
                    lastAimbotShotsFired = currentShotsFired;
                    lastAimbotShotTime = currentTime;
                }
                // Resetuj jeśli gracz przestał strzelać
                else if (currentShotsFired == 0)
                {
                    lastAimbotShotsFired = 0;
                }
            }
            catch { }
        }
        ...
        public static void DrawCircle(int size, Vector4 color)
        {
            ...
            float radius = size;
            GameState.renderer.drawList.AddCircle(new Vector2(GameState.renderer.screenSize.X / 2, GameState.renderer.screenSize.Y / 2), radius, ImGui.ColorConvertFloat4ToU32(circleColor), 32, 1.0f);
        }
